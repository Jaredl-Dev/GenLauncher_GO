...
internal static class BigFileVariantPath
{
    ...
    public static string GetDeploymentPath(string filePath)
    {
        return IsGibFilePath(filePath)
            ? Path.ChangeExtension(filePath, LauncherContentFileTypes.BigExtension)
            : filePath;
    }
    ...
    {
        return Path.ChangeExtension(filePath, LauncherContentFileTypes.GibExtension);
    }
    ...
    {
        return string.Equals(Path.GetExtension(filePath), LauncherContentFileTypes.BigExtension, StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsGibFilePath(string filePath)
    {
        return string.Equals(Path.GetExtension(filePath), LauncherContentFileTypes.GibExtension, StringComparison.OrdinalIgnoreCase);
    }
}
