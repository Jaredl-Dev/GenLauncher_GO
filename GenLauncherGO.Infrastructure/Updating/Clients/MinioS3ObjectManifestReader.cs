...
{
    private readonly Func<S3ObjectManifestRequest, CancellationToken, IAsyncEnumerable<S3ObjectManifestItem>>
        _listObjects;
        ...
        CancellationToken cancellationToken)
    {
        ...
        List<RemoteFileManifestEntry> files = [];
    ...
    [ExcludeFromCodeCoverage(Justification =
        "Wraps MinIO SDK network enumeration; behavior is covered through the injected listing adapter.")]
    private static async IAsyncEnumerable<S3ObjectManifestItem> ListObjectsAsync(
        S3ObjectManifestRequest request,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        using IMinioClient client = MinioClientFactory.Create(
            request.Endpoint,
            request.AccessKey,
            request.SecretKey,
            request.UseSsl);
