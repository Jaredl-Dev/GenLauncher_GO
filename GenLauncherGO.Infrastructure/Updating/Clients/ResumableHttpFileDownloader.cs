...
internal sealed class ResumableHttpFileDownloader : IResumableFileDownloader
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        if (_bufferSize <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(bufferSize), "Download buffer size must be greater than zero.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 1; attempt <= _maxAttempts; attempt++)
        {
            ...
            catch (Exception ex) when (IsRetriable(ex) && attempt < _maxAttempts)
            {
                lastException = ex;
                _logger.LogWarning(
                    ex,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (response.StatusCode == HttpStatusCode.RequestedRangeNotSatisfiable &&
            ...
            existingBytes == request.ExpectedBytes.Value)
        {
            ReportProgress(progress, request.ExpectedBytes, existingBytes);
            return;
        }
