...
internal sealed class S3ReusablePackageFileCopier
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        FileSystemPathSafety.ResolveOwnedSubpath(
            ...
            sourcePath.FullPath,
            "Reusable package paths must remain below launcher-owned content.",
        ...
        FileSystemPathSafety.ResolveOwnedSubpath(
            ...
            "Package staging paths must remain below launcher-owned temporary storage.",
        ...
        IReadOnlyList<RemoteFileManifestEntry> repositoryFiles)
    {
        ...
        foreach (RemoteFileManifestEntry repositoryFile in repositoryFiles)
        {
            ...
            string installedPath = BigFileVariantPath.GetInstalledPath(normalizedPath);
        ...
        CancellationToken cancellationToken)
    {
        FileSystemPathSafety.EnsureExistingPathChainHasNoReparsePoints(
            sourceDir,
            "Reusable package paths must be rooted.",
            "Reusable package paths must not contain reparse points.");
        FileSystemPathSafety.EnsureExistingPathChainHasNoReparsePoints(
            destinationDir,
            "Package staging paths must be rooted.",
            "Package staging paths must not contain reparse points.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(targetFilePath) ||
            !repositoryFileIndex.TryGetValue(relativeFilePath, out RemoteFileManifestEntry? repositoryFile) ||
            repositoryFile.Size != fileSize ||
            !S3HashValidationPolicy.IsReliableMd5Hash(repositoryFile.Hash))
        {
            return;
        }
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(targetFilePath) ?? destinationDir);
