...
internal sealed class PackageDownloadService : IPackageDownloadService
{
    private static readonly HashSet<string> _extensionsToCheckHash =
        ...
        {
            ".w3d",
            BigFileVariantPath.BigExtension,
            ".bik",
            BigFileVariantPath.GibExtension,
            ".dds",
            ".tga",
            ".ini",
            ".scb",
        ...
        PackageDownloadPauseController? pauseController = null)
    {
        ...
        try
        {
            ...
            if (ShouldUseSingleFileDownload(version))
            {
    ...
    private async Task DownloadSingleFilePackageAsync(
        ...
        CancellationToken cancellationToken)
    {
        string downloadUrl = DownloadLinkResolver.ResolveDirectDownloadLink(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await pauseController.WaitWhilePausedAsync(cancellationToken).ConfigureAwait(false);
        LauncherContentVersion? latestInstalledVersion = modification.Versions
    ...
    private PackageUpdatePathSet CreatePackagePaths(
        LauncherPaths paths,
        LauncherContentVersion version,
    ...
    private OwnedContentPath ResolveVersionPath(
        LauncherPaths paths,
        LauncherContentVersion version)
    {
    ...
    private static bool ShouldUseSingleFileDownload(LauncherContentVersion version)
    {
        return version.EffectiveContentSourceKind != ContentSourceKind.ManagedS3;
    }
