...
internal sealed class SingleFilePackageUpdater : ISingleFilePackageUpdater
{
        ...
        string fileName)
    {
        ...
        if (trimmedFileName is "." or ".." ||
            ...
            trimmedFileName.Contains(Path.DirectorySeparatorChar, StringComparison.Ordinal) ||
            trimmedFileName.Contains(Path.AltDirectorySeparatorChar, StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                "Remote package metadata did not provide a safe direct file name.");
        }
