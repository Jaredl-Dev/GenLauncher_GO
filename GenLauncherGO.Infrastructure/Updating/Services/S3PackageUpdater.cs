...
using MinioClientFactory = GenLauncherGO.Infrastructure.Updating.Clients.MinioClientFactory;
...
internal sealed class S3PackageUpdater : IS3PackageUpdater
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Disposed in the finally below rather than by `using` declarations: both are
        // captured by the download tasks, so they must outlive every task instead of
        // being released when this scope exits.
        ...
        {
            // Task.WhenAll surfaces the first failure or cancellation immediately, while the
            // remaining downloads are still running and still holding the semaphore and the
            // client. Every task has to settle before either is released, rather than only
            // the one that failed.
            foreach (Task downloadTask in downloadTasks)
            {
                await downloadTask.ConfigureAwait(ConfigureAwaitOptions.SuppressThrowing);
            }
