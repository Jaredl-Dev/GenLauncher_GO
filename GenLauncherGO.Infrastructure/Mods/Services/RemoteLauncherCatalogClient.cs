...
internal sealed class RemoteLauncherCatalogClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await ReadWithConcurrencyLimitIfAvailableAsync(
            semaphore,
            token => DownloadModDataAsync(reference, token),
            exception =>
                _logger.LogWarning(
        ...
        CancellationToken cancellationToken)
    {
        return await ReadWithConcurrencyLimitIfAvailableAsync(
            semaphore,
            token => ReadModificationYamlAsync(url, parentContentName, token),
            exception =>
                _logger.LogWarning(
    ...
    private static async Task<T?> ReadWithConcurrencyLimitIfAvailableAsync<T>(
        SemaphoreSlim semaphore,
        ...
        where T : class
    {
        ...
        try
        {
            return await ReadIfAvailableAsync(read, logFailure, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            semaphore.Release();
    ...
    private static async Task<T?> ReadIfAvailableAsync<T>(
        Func<CancellationToken, Task<T>> read,
        Action<Exception> logFailure,
        CancellationToken cancellationToken)
        where T : class
    ...
    public async Task<LauncherContentVersion?> DownloadAdvertisingInfoAsync(
        ...
        CancellationToken cancellationToken)
    {
        return await ReadIfAvailableAsync(
            token => ReadModificationYamlAsync(manifestUrl, null, token),
            exception =>
                _logger.LogWarning(
