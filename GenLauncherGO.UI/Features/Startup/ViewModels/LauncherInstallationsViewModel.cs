...
internal sealed class LauncherInstallationsViewModel : ObservableObject
{
        ...
        ILauncherStringLocalizer stringLocalizer)
    {
        ...
        BrowseZeroHourCommand = new AsyncRelayCommand<object?>(
            ...
            owner => owner is Window,
            AsyncRelayCommandOptions.AllowConcurrentExecutions);
        DetectGeneralsCommand = new RelayCommand(
            () => Detect(SupportedGame.Generals),
            () => !IsGeneralsValid);
        DetectZeroHourCommand = new RelayCommand(
            () => Detect(SupportedGame.ZeroHour),
            () => !IsZeroHourValid);
    ...
    public LauncherInstallations CreateValidatedInstallations()
    {
        if (!CanContinue)
        {
            throw new InvalidOperationException("At least one valid installation and no invalid nonempty path are required.");
        }
        ...
        GameInstallationValidationResult validation)
    {
        ...
        string key = validation.Failure switch
        {
            GameInstallationValidationFailure.DirectoryNotFound => "InstallationDirectoryNotFound",
            GameInstallationValidationFailure.RequiredFilesMissing => game == SupportedGame.Generals
                ? "InvalidGeneralsInstallation"
                : "InvalidZeroHourInstallation",
            GameInstallationValidationFailure.LauncherLocationOverlapsGame => "LauncherInsideGameValidation",
            GameInstallationValidationFailure.UnsafeFileSystemPath => "UnsafeInstallationPath",
    ...
    private void NotifyInstallationStateChanged(string changedPathProperty)
    {
        ...
        DetectGeneralsCommand.NotifyCanExecuteChanged();
        DetectZeroHourCommand.NotifyCanExecuteChanged();
    }
