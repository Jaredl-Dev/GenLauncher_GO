...
internal sealed class LauncherGameSessionCoordinator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (targetGame == activeGame &&
            PathsEqual(activeConfiguredPath, _runtimeContext.LauncherPaths.GameDirectory))
        {
            return true;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (oldPaths.Game == newPaths.Game &&
            PathsEqual(oldPaths.GameDirectory, newPaths.GameDirectory))
        {
        ...
        try
        {
            ...
            _pathResolver.PrepareGameDirectories(newPaths, cleanTemporaryDirectory: true);
            ...
            LauncherThemeResourceApplier.Apply(owner, _runtimeContext.Colors);
        ...
        Window owner)
    {
        ...
        {
            RollBackPreferences(rollbackInstallations, rollbackSelectedGame);
            await ShowSwitchFailureAsync(owner, _stringLocalizer["PreferencesSaveFailedDetails"]);
            return false;
        }
    }
    ...
    private Task ShowSwitchFailureAsync(Window owner, string details)
    {
        return _dialogService.ShowErrorAsync(
            new LauncherInfoDialogRequest(
                ...
                string.Format(_stringLocalizer["GameSwitchFailedDetails"], details)),
            owner);
    }
