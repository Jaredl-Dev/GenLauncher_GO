...
public sealed class WindowsGameInstallationRegistryTests
{
    ...
    [Fact]
    public void ReadCandidates_QueriesGeneralsSourcesInGeneralsGameCodeOrder()
    {
    ...
    [Fact]
    public void ReadCandidates_QueriesZeroHourSourcesInGeneralsGameCodeOrder()
    {
        var reads = new List<(RegistryView View, string KeyName, string ValueName)>();
        int candidateNumber = 0;
        var registry = new WindowsGameInstallationRegistry((view, keyName, valueName) =>
    ...
    [Fact]
    public void ReadCandidates_PrefersZeroHourInstallRootOverSteamDataSubdirectory()
    {
        const string InstallRoot = @"C:\Steam\Command & Conquer Generals - Zero Hour";
        const string SteamDataDirectory =
            @"C:\Steam\Command & Conquer Generals - Zero Hour\ZH_Generals";
        var registry = new WindowsGameInstallationRegistry((_, keyName, _) => keyName switch
        {
            ZeroHourEaKey => InstallRoot,
            ZeroHourSteamKey => SteamDataDirectory,
            _ => null
        });

        IReadOnlyList<string> candidates = registry.ReadCandidates(SupportedGame.ZeroHour);

        candidates.Should().Equal(InstallRoot, SteamDataDirectory);
    }

    [Fact]
    public void ReadCandidates_DeduplicatesEquivalentPathsWithoutChangingPriority()
    {
        string[] values =
        [
            "\"C:\\Steam\\Zero Hour\\\"",
