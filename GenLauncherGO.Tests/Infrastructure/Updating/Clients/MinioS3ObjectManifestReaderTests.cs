...
{
    [Theory]
    [InlineData("ShockWave/1.2")]
    [InlineData("ShockWave/1.2/")]
    public async Task ReadManifestAsync_ReturnsPrefixRelativeEntriesAsync(string prefix)
    {
        S3ObjectManifestRequest? receivedRequest = null;
        using CancellationTokenSource cancellationTokenSource = new();
        S3ObjectManifestRequest request = CreateRequest(prefix);
