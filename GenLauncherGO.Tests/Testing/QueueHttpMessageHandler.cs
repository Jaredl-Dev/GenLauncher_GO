...
internal sealed class QueueHttpMessageHandler : HttpMessageHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!_responses.TryDequeue(out Func<HttpRequestMessage, HttpResponseMessage>? responseFactory))
        {
            throw new InvalidOperationException(
                $"No response was configured for {request.Method} {request.RequestUri}.");
        }
