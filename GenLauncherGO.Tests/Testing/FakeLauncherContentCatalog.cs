...
internal sealed class FakeLauncherContentCatalog : ILauncherContentCatalog
{
    ...
    public List<(Uri? RemoteManifestUri, LauncherPaths Paths)> InitializationRequests { get; } = [];
    ...
    public Func<Uri?, LauncherPaths, CancellationToken, Task>? InitializationHandler
    {
        get;
        set;
    }
    ...
    {
        InitializationRequests.Add((remoteManifestUri, paths));
        return InitializationHandler?.Invoke(remoteManifestUri, paths, cancellationToken) ?? Task.CompletedTask;
    }
    ...
    public void DiscardContent(LauncherContentKey contentKey)
    {
        ...
        IReadOnlyList<LauncherContentKey> dependentKeys = Data.GetDependentContentKeys(contentKey);
