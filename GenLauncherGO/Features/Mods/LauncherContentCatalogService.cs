...
internal sealed class LauncherContentCatalogService : ILauncherContentCatalog
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await RunWithCatalogMutationGateAsync(
            async () =>
            {
                CatalogSessionState previousState = _state;
                try
                {
                    _state = new CatalogSessionState(paths);
                    await InitializeDataCoreAsync(remoteManifestUri, cancellationToken).ConfigureAwait(false);
                }
                catch
                {
    ...
    {
        await RunWithCatalogMutationGateAsync(
            () => ReadOriginalGameAddonsAndPatchesCoreAsync(cancellationToken),
            cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await RunWithCatalogMutationGateAsync(
            async () =>
            {
    ...
    public void DiscardContent(LauncherContentKey contentKey)
    {
        IReadOnlyList<LauncherContentKey> dependentKeys =
            _localContentReconciler.DiscardContent(_state.Data, contentKey, Paths);
        PruneDiscardedContentSessionState(contentKey, dependentKeys);
        UpdateLocalModificationsData();
    }

    private void PruneDiscardedContentSessionState(
        LauncherContentKey contentKey,
        IReadOnlyList<LauncherContentKey> dependentKeys)
    {
        var discardedCardKeys = dependentKeys
            .Append(contentKey)
            .Select(key => key.WithoutVersion())
            .ToHashSet();

        _state.DownloadedRepositoryContent.RemoveWhere(candidate =>
            discardedCardKeys.Contains(candidate.WithoutVersion()));
        _state.DownloadedModificationInfo.RemoveWhere(candidate =>
            discardedCardKeys.Contains(candidate.WithoutVersion()));

        foreach (LauncherContentKey cardKey in discardedCardKeys)
        {
            _state.ModificationsAndAddons.Remove(cardKey);
            _state.RepositoryMetadataCache.Remove(cardKey);
        }
    }
    ...
    {
        await RunWithCatalogMutationGateAsync(
            () => ReadPatchesAndAddonsForModCoreAsync(modificationKey, cancellationToken),
            cancellationToken).ConfigureAwait(false);
    }
    ...
    private async Task RunWithCatalogMutationGateAsync(
        Func<Task> operation,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<T> RunWithCatalogMutationGateAsync<T>(
    ...
    {
        _state.Data = LauncherContentStateMapper.ToLauncherData(_contentStateStore.Load(Paths), _themeCache);
    }
    ...
    private async Task InitializeDataCoreAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (remoteManifestUri is not null)
        {
            await ReadMainManifestAsync(remoteManifestUri, cancellationToken).ConfigureAwait(false);
        }
    ...
    /// <summary>
    ///     Loads a content card's patch and add-on manifests concurrently and records a complete load only when both
    ///     groups succeeded. Successful partial results remain available while a later call retries failed manifests.
    /// </summary>
    private async Task LoadChildContentAsync(
        LauncherContentKey contentKey,
        IEnumerable<string> patchManifestUrls,
        IEnumerable<string> addonManifestUrls,
        string? parentContentName,
        CancellationToken cancellationToken)
    {
        Task<RemoteChildManifestLoadResult> patchesTask = _remoteCatalogClient.ReadChildManifestsAsync(
            patchManifestUrls,
            parentContentName,
            cancellationToken);
        Task<RemoteChildManifestLoadResult> addonsTask = _remoteCatalogClient.ReadChildManifestsAsync(
            addonManifestUrls,
            parentContentName,
            cancellationToken);
        RemoteChildManifestLoadResult[] childManifestLoads =
            await Task.WhenAll(patchesTask, addonsTask).ConfigureAwait(false);
        ...
        {
            _state.DownloadedModificationInfo.Add(contentKey);
        }
    }
    ...
    {
        var key = LauncherContentKey.ForModificationName(manifest.Content.Name);
        _state.ModificationsAndAddons.TryAdd(key, manifest);
    }
    ...
    {
        return data.AllContent.Sum(content => content.Versions.Count);
    }
    ...
    private sealed class CatalogSessionState
    {
        ...
        public Dictionary<LauncherContentKey, RemoteModificationManifest> RepositoryMetadataCache { get; } = [];

        public HashSet<LauncherContentKey> DownloadedModificationInfo { get; } = [];

        public HashSet<LauncherContentKey> DownloadedRepositoryContent { get; } = [];
