...
namespace GenLauncherGO.Features.Launcher.Views;
...
internal partial class MainWindow : Window
{
    ...
    private readonly LauncherApplicationUpdateCoordinator _applicationUpdateCoordinator = null!;
    ...
    public MainWindow(
        ...
        LauncherApplicationUpdateCoordinator applicationUpdateCoordinator,
        ...
        : this()
    {
        ...
        _applicationUpdateCoordinator = applicationUpdateCoordinator ??
                                        throw new ArgumentNullException(nameof(applicationUpdateCoordinator));
        ...
        Activated += MainWindow_ActivatedAsync;
    ...
    {
        _contentController.RestoreModsListVerticalOffset();
        await ShowRetailClientRecommendationIfNeededAsync();
        await ExecuteWindowOperationAsync(
            "checking for launcher application updates",
            () => _applicationUpdateCoordinator.StartAsync(this, _windowLifetime.Token));
    }

    private async void MainWindow_ActivatedAsync(object? sender, EventArgs eventArgs)
    {
        _viewModel.RefreshGameClientOptions();
        _viewModel.RefreshWorldBuilderOptions();
        await ExecuteWindowOperationAsync(
            "showing a downloaded launcher application update",
            () => _applicationUpdateCoordinator.HandleLauncherActivatedAsync(this));
    }
    ...
    /// <summary>
    ///     Handles the context request event early to prevent Avalonia from initializing an empty context menu
    ///     popup, which renders a transient 1-pixel visual artifact at the mouse cursor.
    ///     Paired with <see cref="TileContextMenu_Opening"/> as intentional dual defense; do not simplify or remove either.
    /// </summary>
    private void TileRoot_ContextRequested(object? sender, ContextRequestedEventArgs eventArgs)
    {
        if (sender is not Control { DataContext: ModificationViewModel { HasContextMenu: true } })
        {
            eventArgs.Handled = true;
        }
    }
    ...
    /// <summary>
    ///     Guards against opening an empty context menu popup during programmatic or keyboard-driven invocation
    ///     where <see cref="TileRoot_ContextRequested"/> was not triggered. Paired with
    ///     <see cref="TileRoot_ContextRequested"/> as intentional dual defense; do not simplify or remove either.
    /// </summary>
    private void TileContextMenu_Opening(object? sender, CancelEventArgs eventArgs)
    {
        ...
        ModificationViewModel? viewModel = ResolveTileViewModel(menu);
        if (viewModel is { HasContextMenu: false })
        {
            eventArgs.Cancel = true;
            return;
        }

        if (viewModel is null && !menu.Items.OfType<MenuItem>().Any(item => item.IsVisible))
        {
            eventArgs.Cancel = true;
        }
    }

    private static ModificationViewModel? ResolveTileViewModel(ContextMenu menu)
    {
        return (menu.DataContext as ModificationViewModel)
            ?? (menu.PlacementTarget as Control)?.DataContext as ModificationViewModel;
    }
    ...
    private async void MainWindow_ClosingAsync(object? sender, WindowClosingEventArgs eventArgs)
    {
        ...
        UiOperationOutcome outcome = await _exceptionBoundary.ExecuteAsync(
            ...
            async () =>
            {
                ...
                _contentController.SaveModsListVerticalOffset();
    ...
    private async void OpenModificationFolder_ClickAsync(object? sender, RoutedEventArgs eventArgs)
    {
        await ApplyModificationActionAsync(
            sender,
            "opening a modification folder",
            _contentActionCoordinator.OpenModificationFolder);
    }
