...
internal sealed class WindowsProcessFamilyLauncher : IProcessFamilyLauncher
{
    ...
    private const int ProcessHandoffGraceMilliseconds = 500;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var processFamily = new ProcessFamilyTracker(process.Id, executableName, _logger);
        return new WindowsProcessFamilyLaunchOperation(
            processFamily.CurrentExecutableName,
            process,
            processFamily,
            _logger,
            cancellationToken);
        ...
        string workingDirectory)
    {
        ...
        }) ?? throw new InvalidOperationException($"Failed to start {Path.GetFileName(executableName)}.");
        return process;
    }

    /// <summary>
    ///     Returns <see langword="null" /> when ToolHelp cannot capture a snapshot so tracking can fall back to the root
    ///     process.
    /// </summary>
    ...
    internal sealed class ProcessFamilyTracker
    {
        ...
        private readonly Dictionary<int, TrackedProcess> _trackedProcesses = [];
            ...
            Action<int> forceCloseProcess)
        {
            ...
            _startedAtUtc = _timeProvider.GetUtcNow().UtcDateTime;
            ...
            _trackedProcesses.Add(
                rootProcessId,
                new TrackedProcess(_currentExecutableName, 0, _nextProcessOrder++, _startedAtUtc));
        }
        ...
        private bool IsRunningCore()
        {
            ...
            TrackDescendants(entries, nowUtc);
            ...
            // A process that remained visible for at least the full handoff window was the stable launched client, not
            // a transient launcher. Once it exits, report completion immediately instead of adding another grace delay.
            if (!ShouldWaitForHandoff(nowUtc))
            {
                return false;
            }
        ...
        private IReadOnlyList<int> GetTrackedRunningProcessIds()
        {
            ...
            TrackDescendants(entries, _timeProvider.GetUtcNow().UtcDateTime);
            var runningProcessIds = entries
                .Select(entry => entry.ProcessId)
                .ToHashSet();
        ...
        private void TryForceCloseProcess(int processId)
        {
            try
            {
                ...
                _logger.LogInformation("Force closed launched process {ProcessId}.", processId);
            }
            catch (ArgumentException)
            {
                _logger.LogDebug(
            ...
            {
                _logger.LogWarning(exception, "Failed to force close launched process {ProcessId}.", processId);
            }
        ...
        private void TrackDescendants(IReadOnlyList<ProcessSnapshotEntry> entries, DateTime observedAtUtc)
        {
            bool addedProcess;
            do
            {
        ...
        private bool ShouldWaitForHandoff(DateTime nowUtc)
        {
            TrackedProcess? lastRetiredChild = _trackedProcesses.Values
                .Where(process => process.Depth > 0 && process.RetiredAtUtc.HasValue)
                .OrderByDescending(process => process.Depth)
                .ThenByDescending(process => process.Order)
                .FirstOrDefault();
            return lastRetiredChild is not null &&
                   nowUtc - lastRetiredChild.ObservedAtUtc < _handoffGracePeriod;
        }
        ...
        private bool IsRootProcessRunning()
        {
            ...
            _lastObservedRunningAtUtc = _timeProvider.GetUtcNow().UtcDateTime;
            return true;
        }
        ...
        {
            return Path.GetFileName(executableName?.Trim() ?? string.Empty);
        }

        private sealed class TrackedProcess
        {
            public TrackedProcess(string executableName, int depth, int order, DateTime observedAtUtc)
            {
