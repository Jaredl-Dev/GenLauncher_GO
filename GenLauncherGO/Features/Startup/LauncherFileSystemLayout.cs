...
/// </summary>
internal static class LauncherFileSystemLayout
{
    ...
    /// <summary>
    ///     Normalizes a game-root file name or a fully qualified executable path. Relative subpaths are not accepted.
    /// </summary>
    public static string NormalizeExecutablePath(string? executablePath)
    {
        ...
        {
            return NormalizeExecutableFileName(trimmedPath);
        }

        string fullPath = LexicalPath.NormalizeFullPath(trimmedPath);
        NormalizeExecutableFileName(Path.GetFileName(fullPath));
        return fullPath;
    }

    /// <summary>
    ///     Resolves a registered executable independently of deployment containment.
    /// </summary>
    public static string ResolveExecutablePath(string gameDirectory, string executablePath)
    {
        return LexicalPath.ResolvePath(gameDirectory, NormalizeExecutablePath(executablePath));
    }

    /// <summary>
    ///     Keeps game-root executables portable when the deployment folder changes; other locations remain absolute.
    /// </summary>
    public static string NormalizeExecutableRegistration(string? gameDirectory, string executablePath)
    {
        ...
        return !string.IsNullOrWhiteSpace(gameDirectory) &&
               ...
               LexicalPath.AreEquivalent(Path.GetDirectoryName(normalizedPath), gameDirectory)
