...
internal sealed class LauncherSettingsViewModel : ObservableObject
{
        ...
        ILauncherStringLocalizer stringLocalizer)
    {
        ...
        OpenGeneralsOnlineDiscordCommand =
            new RelayCommand(() => OpenUri(LauncherApplicationDefaults.GeneralsOnlineDiscordUrl));
    ...
    public SupportedGame SwitchTargetGame => PerGame.Select(
        _runtimeContext.CurrentlyManagedGame,
        SupportedGame.ZeroHour,
        SupportedGame.Generals);

    public string SwitchGameButtonText => string.Format(CultureInfo.CurrentCulture,
    ...
    {
        get => _preferences.Shared.EnableDiagnosticLogging;
        set => TryUpdatePreferences(_preferences with
        {
            Shared = _preferences.Shared with { EnableDiagnosticLogging = value }
        });
    }
    ...
    public void RefreshAfterGameManagementChange()
    {
        ...
        OnPropertyChanged(nameof(GameArguments));
        OnPropertyChanged(nameof(WorldBuilderArguments));
    }
        ...
        [CallerMemberName] string? propertyName = null)
    {
        ...
        if (!LauncherPreferencesUpdate.TryApply(_preferencesService, preferences))
        {
    ...
    {
        return _stringLocalizer[PerGame.Select(game, "GeneralsShortName", "ZeroHourShortName")];
    }
