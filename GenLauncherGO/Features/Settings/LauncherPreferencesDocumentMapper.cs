...
/// <summary>
///     Maps the standalone preferences persistence schema once into normalized launcher preferences.
/// </summary>
internal static class LauncherPreferencesDocumentMapper
{
    ...
    public static LauncherPreferencesDocument ToDocument(LauncherPreferences preferences)
    {
        ...
        return new LauncherPreferencesDocument
        {
            ...
            Shared = new LauncherSharedPreferencesDocument
            {
                ...
                EnableDiagnosticLogging = normalized.Shared.EnableDiagnosticLogging,
    ...
    public static LauncherPreferences Normalize(LauncherPreferences preferences)
    {
        ...
        var normalizedInstallations = new LauncherInstallations
        {
        ...
        return new LauncherPreferences
        {
            ...
            Games = new LauncherGamePreferencesSet
            {
                Generals = NormalizeGamePreferences(games.Generals, SupportedGame.Generals, normalizedInstallations.Generals),
                ZeroHour = NormalizeGamePreferences(games.ZeroHour, SupportedGame.ZeroHour, normalizedInstallations.ZeroHour)
            }
        };
    }
    ...
    private static LauncherSharedPreferences MapShared(LauncherSharedPreferencesDocument? shared)
    {
        return shared is null
            ...
            : new LauncherSharedPreferences
            {
                ...
                EnableDiagnosticLogging = shared.EnableDiagnosticLogging,
        ...
        string? gameDirectory)
    {
        ...
        return preferences with
        {
            ...
            SelectedGameClient = NormalizeSelectedExecutable(gameDirectory, preferences.SelectedGameClient),
            SelectedWorldBuilder = NormalizeSelectedExecutable(gameDirectory, preferences.SelectedWorldBuilder),
        ...
        IReadOnlyList<LauncherCustomExecutable> executables)
    {
        return executables
            .Select(executable => new LauncherCustomExecutableDocument
            {
                ...
                ExecutableName = executable.ExecutablePath
            })
            .ToList();
    }
        ...
        string? gameDirectory)
    {
        ...
        foreach (LauncherCustomExecutable? executable in executables)
        {
            ...
            string path = LauncherFileSystemLayout.NormalizeExecutableRegistration(gameDirectory, executable.ExecutablePath);
            if (!displayNames.Add(executable.DisplayName) ||
                !executableNames.Add(path))
            {
                changed = true;
                continue;
            }

            bool pathChanged = path != executable.ExecutablePath;
            changed |= pathChanged;
            normalized.Add(!pathChanged
                ? executable
                : new LauncherCustomExecutable(executable.DisplayName, path));
        }

        return changed ? normalized : executables;
    }

    private static string NormalizeSelectedExecutable(string? gameDirectory, string? executablePath)
    {
        ...
        try
        {
            return LauncherFileSystemLayout.NormalizeExecutableRegistration(gameDirectory, executablePath);
        }
        catch (Exception exception) when (exception is ArgumentException or IOException or NotSupportedException)
        {
            return string.Empty;
