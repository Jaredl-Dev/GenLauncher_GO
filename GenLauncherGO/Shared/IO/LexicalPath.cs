...
/// </remarks>
internal static class LexicalPath
{
    /// <summary>
    ...
    /// <summary>
    ///     Determines whether directory trees are identical or one contains the other. Invalid paths do not overlap.
    /// </summary>
    public static bool AreOverlapping(string? left, string? right)
    {
        if (string.IsNullOrWhiteSpace(left) || string.IsNullOrWhiteSpace(right))
        {
            return false;
        ...
        try
        {
            string normalizedLeft = NormalizeFullPath(left);
            string normalizedRight = NormalizeFullPath(right);
            return IsNormalizedPathInDirectory(normalizedLeft, normalizedRight) ||
                   IsNormalizedPathInDirectory(normalizedRight, normalizedLeft);
        }
        catch (Exception exception) when (exception is ArgumentException or IOException or NotSupportedException)
        {
            return false;
    ...
    /// </summary>
    public static string NormalizePathSegment(string? segment, string paramName)
    {
