...
namespace GenLauncherGO.Shared.Persistence;

/// <summary>
///     Commits complete files atomically within their destination directory.
/// </summary>
internal interface IAtomicFileWriter
{
    ...
    /// <summary>
    ///     Writes to a temporary sibling path and atomically publishes it only when the destination is still missing.
    /// </summary>
    /// <returns><see langword="true" /> when this call published the file; otherwise, <see langword="false" />.</returns>
    Task<bool> WriteFileIfMissingAsync(
        string destinationPath,
