...
internal sealed class AtomicFileWriter : IAtomicFileWriter
{
    ...
    public async Task<bool> WriteFileIfMissingAsync(
        string destinationPath,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await writeTemporaryFileAsync(temporaryPath, cancellationToken).ConfigureAwait(false);
            cancellationToken.ThrowIfCancellationRequested();
            return CommitTemporaryFileIfMissing(temporaryPath, fullDestinationPath);
        }
        finally
        {
            if (File.Exists(temporaryPath))
    ...
    private static bool CommitTemporaryFileIfMissing(string temporaryPath, string destinationPath)
    {
        FileSystemPathSafety.EnsureExistingPathChainHasNoReparsePoints(
            destinationPath,
            "Atomic file paths");
