...
namespace GenLauncherGO.Shared.Logging;

/// <summary>
///     Formats log events while removing local paths and obvious secret-bearing URL values.
/// </summary>
internal sealed partial class SensitiveDataRedactingTextFormatter : ITextFormatter
{
    /// <summary>
    ///     Replaces source-file paths emitted by exception stack traces.
    /// </summary>
    private static readonly Regex _stackTraceSourcePathPattern = StackTraceSourcePathPattern();

    /// <summary>
    ///     Replaces UNC paths before drive-letter paths so adjacent path values cannot consume the UNC introducer.
    /// </summary>
    private static readonly Regex _uncWindowsPathPattern = UncWindowsPathPattern();

    /// <summary>
    ///     Replaces absolute drive-letter paths using either Windows path separator.
    /// </summary>
    private static readonly Regex _absoluteDriveWindowsPathPattern = AbsoluteDriveWindowsPathPattern();

    /// <summary>
    ///     Replaces URI user-info credentials.
    /// </summary>
    private static readonly Regex _uriUserInfoPattern = UriUserInfoPattern();

    /// <summary>
    ///     Replaces common token, key, credential, secret, signature, and password query-string values.
    /// </summary>
    private static readonly Regex _sensitiveQueryValuePattern = SensitiveQueryValuePattern();
    ...
    [GeneratedRegex(@"\sin\s[A-Za-z]:\\[^\r\n]*:line\s(?<line>\d+)", RegexOptions.Compiled)]
    private static partial Regex StackTraceSourcePathPattern();

    [GeneratedRegex(@"\\\\[^\\/\r\n:*?""<>|]+[\\/][^\\/\r\n:*?""<>|]+(?:[\\/][^\\/\r\n:*?""<>|]*)*",
        RegexOptions.Compiled)]
    private static partial Regex UncWindowsPathPattern();

    [GeneratedRegex(@"(?<![A-Za-z0-9])[A-Za-z]:[\\/](?:[^\\/\r\n:*?""<>|]+[\\/])*[^\\/\r\n:*?""<>|]*",
        RegexOptions.Compiled)]
    private static partial Regex AbsoluteDriveWindowsPathPattern();

    [GeneratedRegex(@"(?i)(?<scheme>\b[a-z][a-z0-9+.-]*://)[^/\s?#@]+@", RegexOptions.Compiled)]
